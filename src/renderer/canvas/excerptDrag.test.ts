...
describe('excerptCardFromDrop', () => {
  it('builds excerpt card input from payload + board point', () => {
  ...
  it('forwards sourceAnnotationId when present', () => {
    const card = excerptCardFromDrop(
      { source: 'd.md', anchor: { start: 0, end: 1, exact: 'a', prefix: '', suffix: '' }, snapshot: 'a', color: '#fee', sourceAnnotationId: 'ann-1' },
      { x: 5, y: 6 }
    )
